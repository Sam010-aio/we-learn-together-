...
-- Idempotent. NACH 01-feed.sql ausführen (nutzt Bucket `media`, app_flags UND profiles.role).

-- Defensiv (falls 02 isoliert läuft): profiles.role muss existieren, weil die Policies unten
-- darauf verweisen. In der normalen Reihenfolge hat 01 das bereits angelegt.
...
create table if not exists public.projects (
  ...
  contact_release boolean not null default false,             -- default AUS: Uni-Mail erst nach Freigabe
...
-- https-Zwang für alle Link-URLs (server-seitig, nicht nur Client) + updated_at pflegen
...
-- Companies lesen NUR companies_too (Rolle wird in 03 vergeben):
...
-- 4) insert mit http://-Link ⇒ Exception 'project links must be https' (Client blockt zusätzlich).
